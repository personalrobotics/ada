...
def block_grasp(robot, block, manip=None):
    ...
    Ada
      Here we are trying to do different thing
      1. go to the top of the block
      2. move downward and pick up the block
    '''

    # import openravepy
    # h = openravepy.misc.DrawAxes(manip.GetRobot().GetEnv(),test)

    # this is boundary, it is a matrix to store the upper and lower bound of x,y,z,roll,pitch,yaw
    Bw = numpy.zeros((6,2))
